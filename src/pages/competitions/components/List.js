...
class List extends PureComponent {
  ...
  paginate = search => {
    ...
    if (search) {
      ...
      dispatch({ type: 'competitions/list', payload })
    } else {
      payload.limit = this.state.pagination.pageSize
  ...
  onPaginationChange = (options, filters, sorter) => {
    ...
    history.replace(`/competitions${queryUrl || ''}`)
  }

  confirmRemove = record => {
    const { dispatch } = this.props
    dispatch({
      type: 'competitions/removeCompetitions',
      payload: {
        id: record.id,
      },
    })
        ...
        render: (text, record) => (
          <div>
            <Link
              ...
              to={`/competitions/edit/${record.id}`}
            >
              Edit
            </Link>
            <Link
              className={styles.editLink}
              to={`/competitions/view/${record.id}`}
            >
              View
            </Link>
            <Popconfirm
              style={{ height: '100px' }}
              title={<p> Are you sure you want to delete this Competition? </p>}
        ...
        <Row type="flex" justify="center">
          <Col lg={24} md={24} sm={24} xs={24}>
            <Table
              ...
              loading={effects['competitions/list']}
              scroll={{ x: true }}
