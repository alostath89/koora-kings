...
class List extends PureComponent {
  ...
  paginate = search => {
    ...
    if (search) {
      ...
      dispatch({ type: 'players/list', payload })
    } else {
      payload.limit = this.state.pagination.pageSize
  ...
  onPaginationChange = (options, filters, sorter) => {
    ...
    history.replace(`/players${queryUrl || ''}`)
  }
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: <Trans>Salary</Trans>,
        dataIndex: 'salary',
        key: 'salary',
        width: 150,
      },
      {
        title: <Trans>Team Name</Trans>,
        dataIndex: 'teamName',
        key: 'teamName',
        width: 150,
      },
      ...
      },
      {
        ...
        render: (text, record) => (
          <div>
            <Link className={styles.editLink} to={`/players/edit/${record.id}`}>
              Edit
            </Link>
            <Link className={styles.editLink} to={`/players/view/${record.id}`}>
              View
            </Link>
          </div>
        ),
