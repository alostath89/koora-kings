...
class List extends PureComponent {
  ...
  paginate = search => {
    ...
    if (search) {
      ...
      dispatch({ type: 'slider/list', payload })
    } else {
      payload.limit = this.state.pagination.pageSize
  ...
  onPaginationChange = (options, filters, sorter) => {
    ...
    history.replace(`/slider${queryUrl || ''}`)
  }
  ...
  render() {
    ...
    const columns = [
      {
        title: <Trans>Slider Page</Trans>,
        dataIndex: 'slider_place',
        key: 'slider_place',
        render: (text, record) =>
          text === 'home'
            ? 'Home Page'
      ...
      {
        title: <Trans>Images Count</Trans>,
        ...
        render: (text, record) => (
          <div>
            <Link
              ...
              to={`/slider/edit/${record.slider_place}`}
            >
              Edit
            </Link>
          </div>
