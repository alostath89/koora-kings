...
import styles from './SponsoredContests.less'
...
class SponsoreContests extends PureComponent {
  constructor(props) {
    super(props)
    this.state = {
      payload: {},
      ...
      this.setState({
        ...
        outputHTML:
          this.props?.details && !_.isEmpty(this.props?.details)
            ? this.props?.details?.prize_text
            : '<p></p>',
        allCountries: this.props.details?.country === 'all',
      })
    }
  }
  ...
  /* cancel form */
  handleCancel = () => {
    this.props.history.push(`/SponsoredContests`)
  }
  ...
  handleSubmit = () => {
    ...
    form.validateFields(async (err, values) => {
      ...
      values = {
        ...
        country: values.countryAll ? 'all' : values.country,
      }
              ...
              <Row type={'flex'} justify={'start'}>
                ...
                <Col lg={24} md={24} sm={24} xs={24}>
                  <Form.Item label={<span> Country </span>}>
                    {getFieldDecorator('countryAll', {
                      initialValue: this.state.allCountries,
                    })(
                      <Checkbox
                        onClick={this.onSelectAllCountries}
                ...
                <Col lg={24} md={24} sm={24} xs={24}>
                  <Form.Item label={<span> Country </span>}>
                    {getFieldDecorator('country', {
                      initialValue:
                        details?.country !== 'all' && details?.country !== ''
                          ? details?.country &&
                            details?.country
                              ?.split(',')
                              .map(row => Number(row))
                      ...
                      rules: [
                        {
                          required: !this.state.allCountries,
